...
function Dashboard({ envelopes, ratios, monthlyIncome, monthExpenses, totalSpent, saved,
  activeEnvTab, setActiveEnvTab, onAddTx, onRemoveCat, onAddCat, addingCat, setAddingCat }) {
    ...
    <React.Fragment>
      <div className="topbar">
        ...
        <div className="topbar-actions">
          <div className="month-switcher">
            <button onClick={() => setMonthOffset(o => o - 1)}><Icon name="chevron-left" size={14} /></button>
            <span className="month-label">{monthLabel(currentMonth)}</span>
            <button onClick={() => setMonthOffset(o => Math.min(0, o + 1))}><Icon name="chevron-right" size={14} /></button>
          </div>
      ...
      <div className="hero-grid">
        ...
        <div className="hero-card">
          ...
          <div className="delta">{((totalSpent/monthlyIncome)*100).toFixed(0)}% des revenus</div>
        </div>
        <div className="hero-card">
      ...
      <div className="section-title" style={{ marginTop: 8 }}>
        Enveloppes <span className="section-title-meta">— Méthode {ratios.necessities}/{ratios.wants}/{ratios.investments}</span>
      </div>
      <div className="envelopes-row">
        {Object.values(envelopes).map(env => {
          ...
          return (
            <div key={env.id} className={`envelope env-${env.id}`} onClick={() => setActiveEnvTab(env.id)}>
              ...
              <div className="envelope-name">{env.label === 'Nécessités' ? 'Vie courante' : env.label === 'Envies' ? 'Plaisirs' : 'Avenir'}</div>
              ...
              <div className="envelope-foot">
                ...
                <span className={`envelope-remaining num ${over ? 'over' : ''}`}>{fmtEUR(Math.abs(remaining))}</span>
              </div>
            </div>
          );
        })}
      ...
      {envelope.categories.map(c => {
        ...
        return (
          <div key={c.id} className="cat-tile">
            <button className="cat-tile-delete" onClick={(e) => { e.stopPropagation(); onRemove(envelope.id, c.id); }} title="Supprimer">
              <Icon name="x" size={12} />
            </button>
            ...
            <div className="cat-tile-bar"><div className="cat-tile-bar-fill" style={{ width: pct + '%' }}></div></div>
            <div className="cat-tile-meta">{budget > 0 ? ((amount/budget)*100).toFixed(0) : 0}% de l'enveloppe</div>
          </div>
        );
      })}
      {adding ? (
