...
const fmtEUR = (n, opts = {}) => {
  ...
  const formatted = compact && abs >= 1000
    ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 0, maximumFractionDigits: abs >= 10000 ? 0 : 1 }).format(abs)
...
function App() {
  ...
  const ratios = { necessities: tweaks.ratioNec, wants: tweaks.ratioWant, investments: tweaks.ratioInv };
  ...
  const addCategory = (envId, name) => {
    setEnvelopes(prev => ({
      ...prev,
      [envId]: { ...prev[envId], categories: [...prev[envId].categories, { id: 'c' + Date.now(), label: name, icon: '◧' }] },
    }));
  };
    ...
    <div className="app">
      ...
      {showAddTx && (
        <AddTxModal
          ...
          onAdd={(tx) => { addTransaction(tx); setShowAddTx(false); }}
          onAddRecurring={(r) => { addRecurring(r); }}
        />
      )}
