  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="nav">
        ...
        <button className={`nav-item ${view==='recurring' ? 'active' : ''}`} onClick={() => setView('recurring')}>
      ...
      <div>
        ...
        {accounts.map(a => (
          <div key={a.id} className={`account-card ${a.type === 'savings' ? 'savings' : ''}`}>
            ...
            <span className="account-card-balance num">€{a.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
        ))}
        <div className="account-card" style={{ background: 'var(--bg-sunk)', boxShadow: 'none', cursor: 'pointer' }} onClick={onEditAccounts}>
          <span className="account-card-label" style={{ color: 'var(--text-subtle)' }}>Total tous comptes</span>
      ...
      <div style={{ marginTop: 'auto', padding: '12px', fontSize: 11, color: 'var(--text-subtle)', lineHeight: 1.5 }}>
