...
function Stats({ envelopes, ratios, monthlyIncome, monthExpenses, transactions }) {
  const totalSpent = monthExpenses.totals.necessities + monthExpenses.totals.wants + monthExpenses.totals.investments;
  ...
  const maxMonthlyTotal = Math.max(...monthlyData.map(m => m.necessities + m.wants + m.investments));
    ...
    <React.Fragment>
      ...
      <div className="stats-grid" style={{ marginTop: 12 }}>
        <div className="card">
          <div className="card-h">
            ...
            <div className="card-title num" style={{ color: 'var(--text-subtle)' }}>Déc → Mai</div>
          </div>
              ...
              return (
                <div className="bars-row" key={m.month}>
                  ...
                  <div className="bars-stack">
                    <span style={{ width: necW + '%', background: 'var(--nec)' }} title={`Nécessités: ${fmtEUR(m.necessities)}`}></span>
                    <span style={{ width: wantW + '%', background: 'var(--want)' }} title={`Envies: ${fmtEUR(m.wants)}`}></span>
                    <span style={{ width: invW + '%', background: 'var(--inv)' }} title={`Investissements: ${fmtEUR(m.investments)}`}></span>
                  </div>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 14, fontSize: 12, color: 'var(--text-muted)' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--nec)' }}></span>Nécessités
            </span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--want)' }}></span>Envies
            </span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--inv)' }}></span>Investissements
            </span>
          </div>
        </div>
      ...
      <div className="card" style={{ marginTop: 12 }}>
        <div className="card-h">
          ...
          <div className="card-title num" style={{ color: 'var(--text-subtle)' }}>{top.length} sur {allCats.length}</div>
        </div>
          ...
          {top.map(c => {
            ...
            return (
              <div className={`top-cat env-${c.envelope.id}`} key={c.id}>
                <div className="top-cat-icon" style={{ background: 'var(--env-soft)', color: 'var(--env)' }}>
                  {c.label.charAt(0)}
                </div>
                <div className="top-cat-info">
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span className="top-cat-name">{c.label}</span>
                    <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{c.envelope.label}</span>
                  </div>
                  <div className="top-cat-bar">
                    <div className="top-cat-bar-fill" style={{ width: pct + '%', background: 'var(--env)' }}></div>
                  </div>
                </div>
  ...
  return (
    <div className="donut-wrap">
      <svg className="donut-svg" width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <text x="50%" y="48%" textAnchor="middle" fontSize="11" fill="var(--text-muted)" fontWeight="500">Dépensé</text>
        <text x="50%" y="58%" textAnchor="middle" fontSize="20" fontFamily="var(--font-mono)" fontWeight="500" fill="var(--text)">
          ...
          return (
            <div className="donut-legend-row" key={d.id}>
              <div className="donut-legend-top">
                ...
                <span className="donut-legend-amount num">{fmtEUR(d.spent, { compact: true })}</span>
              </div>
              <div className="donut-legend-bar">
                <div className="donut-legend-bar-fill" style={{ width: pct + '%', background: d.color }}></div>
              </div>
              <div className="donut-legend-meta">{pct.toFixed(0)}% des dépenses · budget {fmtEUR(d.budget, { compact: true })}</div>
            </div>
          );
        })}
      </div>
