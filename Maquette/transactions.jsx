	...
	return (
		<React.Fragment>
			<div className="topbar">
				<div>
					...
					<div className="topbar-sub">
						{filtered.length} mouvement{filtered.length > 1 ? 's' : ''} · classés automatiquement
					</div>
				</div>
			...
			<div className="tx-toolbar">
				...
				/>
				<button
					className={`tx-filter-pill ${!filterEnv ? 'active' : ''}`}
					onClick={() => setFilterEnv(null)}
				>
					Tout
				</button>
					...
					>
						<span
							style={{
								...
								height: 6,
								borderRadius: '50%',
								background: filterEnv === env.id ? 'currentColor' : 'var(--env)'
							}}
						></span>
						{env.label}
					</button>
									...
									<div key={t.id} className={`tx-row env-${t.envelope || 'necessities'}`}>
										...
										<div>
											...
											<div className="tx-cat">
												{isTransfer && toAcc ? (
													<span className="tx-transfer-arrow">
														{acc?.label} → {toAcc.label}
													</span>
												) : cat ? (
													cat.label
												) : t.income ? (
										...
										<div>
											{env ? (
												...
												>
													<span
														className="tx-envelope-tag-dot"
														style={{ background: 'oklch(0.45 0.10 155)' }}
													></span>
													Revenu
												</span>
											)}
