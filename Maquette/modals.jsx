...
function AddTxModal({ envelopes, accounts, onClose, onAdd, onAddRecurring, defaultMode = 'expense' }) {
  ...
  const submit = (e) => {
    ...
    if (isRecurring) {
      ...
      if (mode !== 'income') { rec.envelope = envelope; rec.category = category; }
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <form className="modal" onClick={(e) => e.stopPropagation()} onSubmit={submit} style={{ position: 'relative' }}>
        ...
        <div className="modal-tabs">
          <button type="button" className={`modal-tab ${mode === 'expense' ? 'active' : ''}`} onClick={() => setMode('expense')}>
            Dépense
          </button>
          <button type="button" className={`modal-tab ${mode === 'transfer' ? 'active' : ''}`} onClick={() => setMode('transfer')}>
            Transfert
          </button>
          <button type="button" className={`modal-tab ${mode === 'income' ? 'active' : ''}`} onClick={() => setMode('income')}>
            Revenu
          </button>
        </div>
        ...
        <div className="field">
          ...
          <input
            type="text"
            placeholder={mode === 'transfer' ? 'Ex. Virement Livret A' : mode === 'income' ? 'Ex. Salaire' : 'Ex. Carrefour Market'}
            value={merchant}
        ...
        {mode === 'transfer' && (
          <div className="transfer-row">
            ...
            <div className="transfer-arrow-icon"><Icon name="chevron-right" size={16} /></div>
            <div className="field" style={{ marginBottom: 0 }}>
              <label>Vers</label>
              <select value={toAccount} onChange={(e) => setToAccount(e.target.value)}>
                {accounts.filter(a => a.id !== account).map(a => <option key={a.id} value={a.id}>{a.label}</option>)}
              </select>
            </div>
          </div>
        )}
        ...
        {mode !== 'income' && (
          <React.Fragment>
            <div className="field" style={{ marginTop: mode === 'transfer' ? 14 : 0 }}>
              <label>Enveloppe {mode === 'transfer' && <span style={{ textTransform: 'none', fontWeight: 400, color: 'var(--text-subtle)' }}>— pour la classification budgétaire</span>}</label>
                ...
                {Object.values(envelopes).map(env => (
                  ...
                  >
                    <span className="envelope-radio-name">
                      <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--env)' }}></span>
                      {env.label}
                    </span>
...
function AccountsModal({ accounts, setAccounts, onClose }) {
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{ position: 'relative', width: 520 }}>
        <button type="button" className="modal-close" onClick={onClose}>
          <Icon name="x" size={16} />
        </button>
        <h2>Comptes</h2>
        ...
        <button type="button" className="btn btn-ghost" onClick={add} style={{ width: '100%', justifyContent: 'center' }}>
